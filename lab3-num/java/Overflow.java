    ...
    */
    private static List<Float> power_fp(float n, int m)
    {
        ...
        for ( int j = 1; j <= m; j++ )
        {
            ...
            // Model answer:
            assert ns * 2 != ns :"Floating-point overflow";

            // Solution 1: Use float method
            //assert !Float.isInfinite(ns) : "Floating-point overflow";
            
            // Solution 2: Testing ns (float) against max float representation
            // assert ns <= Float.MAX_VALUE : "Floating-point overflow";
            
            // Solution 3: Testing ns (float) against larger data type (double)
            // assert Math.pow(n, m) > ns: "Floating-point overflow";
            // ns will reach infinity sooner than double (double maxes out much later)
